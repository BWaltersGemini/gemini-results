...
export default function AdminPage() {
  ...
  const handleRefreshAndPublish = async () => {
    ...
    try {
      const rawResults = await fetchResultsForEvent(selectedEventId);
      ...
      const { error: deleteError } = await supabase
        ...
        .delete()
        .eq('event_id', selectedEventId.toString());
      if (deleteError) {
          ...
          <>
            ...
            {activeTab === 'website' && (
              <>
                ...
                {/* Google Analytics (Forthcoming) */}
                <section className="mb-12">
                  <h2 className="text-3xl font-bold mb-4">Google Analytics (Coming Soon)</h2>
                  <p className="text-gray-600">This section will include Google Analytics integration in the future.</p>
                </section>
              </>
            )}
