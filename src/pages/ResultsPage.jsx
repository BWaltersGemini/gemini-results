// src/pages/ResultsPage.jsx (Final: Safe dates + full width + logos on tiles)
...
export default function ResultsPage() {
      ...
      <div className="min-h-screen bg-gradient-to-b from-gemini-light-gray to-white pt-40 py-20">
        <div className="w-full px-4 sm:px-6 lg:px-8 xl:px-12">
          <div className="text-center mb-20">
            <h1 className="text-5xl md:text-7xl font-black text-gemini-dark-gray mb-6">
              Race Results
            </h1>
            <p className="text-2xl md:text-3xl text-gray-700 max-w-4xl mx-auto">
          ...
          {recentEvents.length > 0 ? (
                ...
                {recentEvents.map((event, index) => (
                  ...
                  >
                    ...
                    {eventLogos[event.id] ? (
                      ...
                      <div className="h-48 bg-gradient-to-br from-gemini-blue to-gemini-dark-gray flex items-center justify-center">
                        <div className="text-white text-6xl opacity-30 group-hover:opacity-50 transition">
                          🏁
                        </div>
                      </div>
                    )}
                    ...
                    <div className="p-8 text-center">
                      <h3 className="text-2xl font-bold text-gemini-dark-gray mb-3 group-hover:text-gemini-blue transition">
                        {event.name}
                      </h3>
                      <p className="text-lg text-gray-600 mb-4">
                        {formatDate(event.date)}
                      </p>
                      <span className="inline-block bg-gemini-red text-white px-6 py-3 rounded-full font-bold text-sm tracking-wider group-hover:bg-gemini-red/90 transition">
                        View Results →
                      </span>
                    </div>
                  </button>
            ...
            <div className="text-center py-20">
              <p className="text-2xl text-gray-600 mb-8">No recent races available</p>
  ...
  return (
    <div className="min-h-screen bg-gemini-light-gray pt-40 py-12">
      <div className="w-full px-4 sm:px-6 lg:px-8 xl:px-12">
        {error && <p className="text-center text-red-600 text-xl font-bold mb-8">{error}</p>}
        ...
        <div className="text-center mb-12">
          ...
          />
          <h1 className="text-4xl sm:text-5xl font-bold text-gemini-dark-gray">{selectedEvent.name}</h1>
          <p className="text-xl sm:text-2xl text-gray-600 mt-2">{formattedDate}</p>
        </div>

        {loadingResults ? (
          <div className="text-center py-20">
            <div className="text-8xl animate-spin inline-block">🏃</div>
            <p className="text-2xl mt-6">Loading results...</p>
          </div>
          ...
          <>
              ...
              <div className="flex flex-wrap justify-center gap-4">
                {racesToShow.map(r => (
                  <button
                    ...
                    className="px-6 py-3 bg-gemini-blue text-white rounded-full hover:bg-gemini-blue/80 font-medium transition"
                  >
                    {r.race_name}
                  </button>
                ))}
            ...
            {racesToShow.map(race => {
              ...
              return (
                ...
                >
                  <h3 className="text-3xl font-bold text-center mb-8">{race.race_name}</h3>

                  {/* Leaderboard */}
                  <div className="grid md:grid-cols-2 gap-8 mb-12 max-w-5xl mx-auto">
                    <div>
                      <h4 className="text-2xl font-bold mb-4">Top Males</h4>
                      {topM.length === 0 ? (
                        <p className="text-gray-500">No male finishers</p>
                      ) : (
                        topM.map((w, i) => (
                          <div key={i} className="mb-4 p-4 bg-white rounded-lg shadow">
                            <p className="font-bold">{i + 1}. {w.first_name} {w.last_name}</p>
                            <p>Time: {w.chip_time || 'N/A'}</p>
                            <p>Age: {w.age || 'N/A'}</p>
                          </div>
                        ))
                      )}
                    ...
                    <div>
                      <h4 className="text-2xl font-bold mb-4">Top Females</h4>
                      {topF.length === 0 ? (
                    ...
                    <div className="text-center mt-8">
                      <div className="flex justify-center items-center gap-6">
                        <button
                          ...
                          className="px-6 py-3 bg-gemini-blue text-white rounded-lg disabled:bg-gray-400 hover:bg-gemini-blue/90 transition"
                        >
                          ← Previous
                        </button>
            ...
            {ads.length > 0 && (
              <div className="mt-16">
                <h3 className="text-3xl font-bold text-center mb-8">Our Sponsors</h3>
